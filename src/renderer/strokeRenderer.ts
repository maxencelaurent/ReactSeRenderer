...
import {EAST, NORTH, SOUTH, WEST} from "../model/Graphic";
  ...
  if (stroke.fill.type === "SolidFill") {
    ...
    allGeoms.forEach(geom => {
      ...
      const strokeArea_L: Geometry = {
        ...
        coordinates: [
          l1.geometry.coordinates,
          l2.geometry.coordinates
        ]
      };
...
export function drawGraphicStroke(
  ...
  geoms.forEach(geom => {
    ...
    chunks.forEach((point) => {
      ...
      drawGraphics(stroke.graphics, false, angle, {type: 'Point', coordinates: point.point}, feature, newContext);
    })
  });
}
